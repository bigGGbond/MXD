...
# 打开 Excel 文件
wb = openpyxl.load_workbook(r'C:\Users\李柱柱\Desktop\scripts\鞋子.xlsx')
ws = wb['Sheet1']
...
for row_idx, row in enumerate(ws.iter_rows(values_only=True)):
    ...
    item_id = str(row[0])            # 当前鞋子ID
    current_step = int(row[1])       # 当前阶数
    level = row[2]                   # 等级（1~10）
    gong_mo = row[3]                 # 攻/魔
    move_speed = row[4]              # 移动速度
    ...
    material2_id = null_if_none(row[11])   # 材料2
    material2_bind = null_if_none(row[12])  # 材料2绑定
    material2_qty = row[13] or 0     # 材料2数量
...
# 保存为 JSON 文件
with open('鞋子奖励数据.json', 'w', encoding='utf-8') as f:
