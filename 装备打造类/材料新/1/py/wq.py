...
# 打开 Excel 文件
wb = openpyxl.load_workbook(r'C:\Users\李柱柱\Desktop\scripts\武器.xlsx')
ws = wb['Sheet1']
...
for row_idx, row in enumerate(ws.iter_rows(values_only=True)):
    ...
    item_id = str(row[0])            # 当前武器ID
    current_step = int(row[1])       # 当前阶数
    level = row[2]                   # 等级（1~10）
    gong_mo = row[3]                 # 攻/魔
    ming_zhong = row[4]              # 命中率
...
# 保存为 JSON 文件
with open('武器奖励数据.json', 'w', encoding='utf-8') as f:
