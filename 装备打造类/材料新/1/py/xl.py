...
def process_excel():
    # 打开Excel文件
    wb = openpyxl.load_workbook(r'C:\Users\Admin\Desktop\sc\scripts\材料新\excel\项链.xlsx')
    ws = wb['Sheet1']
    ...
    for row_idx, row in enumerate(ws.iter_rows(values_only=True)):
        ...
        # 检查行是否为空或数据不完整
        if not row or len(row) < 14:  # 确保至少有所需的所有列
            print(f"警告：第 {row_idx + 1} 行数据不完整或为空: {row}")
            continue

        try:
            # 获取基本数据
            necklace_id = str(row[0])     # 项链ID
            level = int(row[1])           # 等级
            li_min_yun_zhi = row[2]       # 力敏运智
            ...
            # 添加到数据结构
            data[necklace_id][f"等级{level}"] = level_data
...
def save_json(data, filename='项链奖励数据.json'):
    """保存为JSON文件"""
