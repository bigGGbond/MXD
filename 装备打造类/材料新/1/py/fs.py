...
# 打开 Excel 文件
wb = openpyxl.load_workbook(r'C:\Users\Admin\Desktop\sc\scripts\材料新\excel\副手.xlsx')
ws = wb['Sheet1']
...
for row_idx, row in enumerate(ws.iter_rows(values_only=True)):
    ...
    gong_mo = row[4]
    ming_zhong = row[5]
    boss_id = null_if_none(row[6])
    boss_bind = null_if_none(row[7])
    boss_qty = row[8] or 0
    material1_id = null_if_none(row[9])
    material1_bind = null_if_none(row[10])
    material1_qty = row[11] or 0
    material2_id = null_if_none(row[12])
    material2_bind = null_if_none(row[13])
    material2_qty = row[14] or 0
    gold_id = 0
    gold_qty = row[15] or 0
    material_list = [
        [boss_id, boss_qty],
...
# 保存为 JSON 文件
with open('副手奖励数据.json', 'w', encoding='utf-8') as f:
