...
for row_idx, row in enumerate(ws.iter_rows(values_only=True)):
    ...
    material1_bind = null_if_none(row[9])
    material1_qty = row[10] or 0
    material2_id = null_if_none(row[11])
    material2_bind = null_if_none(row[12])
    material2_qty = row[13] or 0
    gold_id = 0
...
for item_id in raw_data:
    ...
    for stage in range(1, 14):  # 1~13
        ...
        next_stage = stage + 1
        next_stage_id = None
        for temp_id, temp_data in raw_data.items():
            if next_stage in temp_data:
                next_stage_id = int(temp_id)
                break
        
        final_data[item_id][stage_key] = {"进阶id": next_stage_id if next_stage_id else int(item_id)}
...
# 保存为 JSON 文件
with open('坐骑奖励数据.json', 'w', encoding='utf-8') as f:
