...
def process_excel():
    wb = openpyxl.load_workbook(r'C:\Users\李柱柱\Desktop\sc\新建文件夹\材料新\材料新\excel\扎昆头盔.xlsx')
    ws = wb['Sheet1']
    ...
    for row_idx, row in enumerate(ws.iter_rows(values_only=True)):
        ...
        # 材料2（只有非绑）
        mat2_unbind = null_if_none(row[12])
        mat2_qty = int(row[13] or 0)
        # 材料3
        mat3_bind = null_if_none(row[14])
        mat3_unbind = null_if_none(row[15])
        mat3_qty = int(row[16] or 0)
        gold = int(row[17] or 0)     # 金币
...
def save_json(data, filename='扎昆头盔进阶数据.json'):
