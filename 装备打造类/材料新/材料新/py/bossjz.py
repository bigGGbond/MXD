...
def process_excel():
    # 打开Excel文件
    wb = openpyxl.load_workbook(r'C:\Users\李柱柱\Desktop\sc\新建文件夹\材料新\材料新\excel\boss戒指.xlsx')
    ws = wb['Sheet1']
    ...
    for row_idx, row in enumerate(ws.iter_rows(values_only=True)):
        ...
        stat = int(row[4] or 0)      # 四维
        atk_mag = int(row[5] or 0)   # 攻魔
        def_mag = int(row[6] or 0)   # 双防
...
def save_json(data, filename='boss戒指进阶数据.json'):
    """保存为JSON文件"""
