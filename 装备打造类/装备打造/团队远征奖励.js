...
var boss事件 = ["clockboss", "ZakumBattle", "HorntailBattle", "PinkBeanBattle", "ChaosZakum", "ChaosHorntail", "boss7", "boss8", "boss9", "boss10", "boss11", "boss12", "boss13", "boss14"];
// var boss事件 = ["boss7", "boss8", "boss9", "boss10", "boss11", "boss12", "boss13", "boss14"];
function start() {
	status = -1;
	action(1, 0, 0);
}
...
function action(mode, type, selection) {
	...
	if (mode == -1) {
		...
		if (status == 0) {
			...
			cm.setBossLog("难度" + 难度, 1);
			当前地图 = cm.getMapId();
			当前地图怪物数量 = cm.getPlayer().getMap().getAllMonstersThreadsafe().size();//怪物数量
			//判断是否打完怪
			if (当前地图怪物数量 != 0) {
				cm.sendOk("清理完当前地图的怪物才可以领取奖励哦！\r\n");
				cm.dispose();
				return;
			}

			// 根据难度获取对应的boss事件
			var 当前事件 = boss事件[难度 - 1];
			if (!当前事件) {
				cm.sendOk("无法找到对应难度的BOSS事件！");
				cm.dispose();
				return;
			}

			// if (getBossLog("难度"+难度)>0) {
			// 	cm.sendNext("您已经领取过该奖励了");
			// 	status=2;
			// }
			var text = "";
			text += "您当前难度为难度" + 难度 + "" + 当前事件 + "\r\n";
			text += "恭喜你击杀BOSS，这是你本次的伤害排名:\r\n";
			text += "  #w名次             名字       伤害  \r\n"
			// var toSpawn = cm.获取BOSS伤害排行(当前事件);
			// for (var i = 0; i < toSpawn.length; i++) {
			// 	if (toSpawn[i].getLeft() == cm.getPlayer().getName()) {
			// 		个人排名 = i + 1;
			// 	}
			// 	text += "#rNO." + (i + 1) + "#k " + toSpawn[i].getValue().getLeft() + " : "+toSpawn[i].getValue().getRight()+ "\r\n";
			// }
			var toSpawn = cm.获取BOSS伤害排行();
			//cm.getPlayer().dropMessage(toSpawn[0]);
			for (var i = 0; i < toSpawn.length; i++) {
				text += "#rNO." + (i + 1) + "#k " + toSpawn[i].getValue().getLeft() + " : " + toSpawn[i].getValue().getRight() + "\r\n";
				if (toSpawn[i].getValue().getLeft() == cm.getPlayer().getName()) {
					个人排名 = i + 1;
				}
			}
			text += "是否领取奖励？\r\n";
			...
			if (个人排名 == -1) {
				cm.sendNext("您对boss伤害为0，无法获取排名奖励");
			} else {
				// 发放奖励
				var text = "";
				text += "\r\n您当前排名是" + 个人排名 + "\r\n";
				...
				cm.setBossLog("难度" + 难度, 1);
				if (cm.getBossLog("团队BOSS奖励倍数") == 1) {
					cm.getPlayer().gainzb(-1000);
				}
				if (当前地图 == 910033304) {
					if (cm.haveItem(1102563, 1)) {
						text += "\r\n#z" + 1102563 + "##v" + 1102563 + "#*1\r\n";
						cm.给属性装备(1102563, 0, 0, 0, 0, 0, 0, 0, 0, 20, 20, 32, 32, 0, 0, 0, 0);
					}
				}
				if (当前地图 == 910033305) {
					if (cm.haveItem(1002357, 1)) {
						cm.给属性装备(1002357, 0, 0, 15, 15, 15, 15, 0, 0, 20, 20, 32, 32, 0, 0, 0, 0);
						text += "\r\n#z" + 1002357 + "##v" + 1002357 + "#*1\r\n";
					}
				}
				if (当前地图 == 910033306) {
					if (cm.haveItem(1122000, 1)) {
						cm.给属性装备(1122000, 0, 0, 20, 20, 20, 20, 400, 400, 30, 30, 0, 0, 0, 0, 0, 0);
						text += "\r\n#z" + 1122000 + "##v" + 1122000 + "#*1\r\n";
					}
				}
				cm.sendOk("恭喜你获得奖励" + text);
			}
