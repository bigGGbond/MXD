...
var 进阶数据 = {
    "802030": {}, "1412011": {}, "1422014": {}, "1432012": {}, "1442024": {}, "1382012": {},
    "1452022": {}, "1462019": {}, "1472032": {}, "1332025": {}, "1482020": {}, "1492020": {},
    "1442225": {
        "等级0": {
            "HP": 0,
            "MP": 0,
...
function action(mode, type, selection) {
    var 初始武器ID列表 = [
        1302030, 1412011, 1422014, 1432012, 1442024, 1382012,
        1452022, 1462019, 1472032, 1332025, 1482020, 1492020
    ];

    // 复制1442225的升级配置到每个初始武器
    for (var i = 0; i < 初始武器ID列表.length; i++) {
        var weaponId = 初始武器ID列表[i];
        if (!进阶数据[weaponId]) {
            进阶数据[weaponId] = {};
        }
        // 复制每个等级的数据
        for (var j = 0; j <= 80; j++) {
            var levelKey = "等级" + j;
            if (进阶数据["1442225"][levelKey]) {
                进阶数据[weaponId][levelKey] = {
                    "HP": 进阶数据["1442225"][levelKey]["HP"],
                    "MP": 进阶数据["1442225"][levelKey]["MP"],
                    "四维": 进阶数据["1442225"][levelKey]["四维"],
                    "攻魔": 进阶数据["1442225"][levelKey]["攻魔"],
                    "双防": 进阶数据["1442225"][levelKey]["双防"],
                    "命中": 进阶数据["1442225"][levelKey]["命中"],
                    "材料": 进阶数据["1442225"][levelKey]["材料"].slice(),
                    "绑定材料": 进阶数据["1442225"][levelKey]["绑定材料"].slice()
                };
            }
        }
    };
    ...
    if (status == 0) {
        ...
        if (当前星级 >= 最高等级) {
            ...
            if (cm.随机数(100) <= 成功率) {
                ...
                item.setAcc(item.getAcc() + 进阶数据[强化物品][下一等级]["命中"]);
                item.setUpgradeSlots(1); // 装备可升级次数
                item.setLevel(1); // 装备已升级次数
