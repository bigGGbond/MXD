...
function setup(eim, leaderid) {
    ...
    em.setProperty("leader", "true");
    var eim = em.newInstance("boss11");
    var map = eim.setInstanceMap(910033308);//刷新地图
    eim.setProperty("boss11Summoned", "0");
    map.resetFully();
    ...
    map.setRankName("boss11");//设置boss名称
...
function gainchrdamage(eim, string) {
    var map = eim.getMapFactory().getMap(910033308);
...
function end(eim) {
    eim.disposeIfPlayerBelow(100, 910033308);//死了出去的地图
    em.setProperty("state", "0");
    em.setProperty("leader", "true");
    em.setProperty("boss11Summoned", "0");
}
...
function allMonstersDead(eim) {
    ...
    gainchrdamage(eim, 'boss11');  // 更新BOSS标识
