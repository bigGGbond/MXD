...
function setup(eim, leaderid) {
    ...
    em.setProperty("leader", "true");
    var eim = em.newInstance("boss9");
    var map = eim.setInstanceMap(910033306);//刷新地图
    eim.setProperty("boss9Summoned", "0");
    map.resetFully();
    ...
    map.setRankName("boss9");//设置boss名称
...
function gainchrdamage(eim, string) {
    var map = eim.getMapFactory().getMap(910033306);
...
function end(eim) {
    eim.disposeIfPlayerBelow(100, 910033306);//死了出去的地图
    em.setProperty("state", "0");
    em.setProperty("leader", "true");
...
function allMonstersDead(eim) {
    ...
    gainchrdamage(eim, 'boss9');  // 更新BOSS标识
    player.dropMessage(-11, "Boss已经击杀");
    // var players = eim.getPlayers();
    // for (var i = 0; i < players.size(); i++) {
    //     var player = players.get(i);
    //     player.warp(1020000, 0);
