...
function setup(eim, leaderid) {
    ...
    em.setProperty("leader", "true");
    var eim = em.newInstance("boss13");
    var map = eim.setInstanceMap(910033310);//刷新地图
    eim.setProperty("boss13Summoned", "0");
    ...
    map.setRankName("boss13");//设置boss名称
...
function gainchrdamage(eim, string) {
    var map = eim.getMapFactory().getMap(910033310);
...
function end(eim) {
    ...
    em.setProperty("state", "0");
    em.setProperty("leader", "true");
    em.setProperty("boss13Summoned", "0");
}
...
function allMonstersDead(eim) {
    ...
    gainchrdamage(eim, 'boss13');  // 更新BOSS标识
    // var players = eim.getPlayers();
    // for (var i = 0; i < players.size(); i++) {
    //     var player = players.get(i);
