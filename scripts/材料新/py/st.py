...
def process_excel():
    # 打开Excel文件
    wb = openpyxl.load_workbook(r'C:\Users\李柱柱\Desktop\sc\scripts\材料新\excel\手套.xlsx')
    ws = wb['Sheet1']

    # 初始化数据结构
    data = {}
    current_glove_id = None
    ...
    for row_idx, row in enumerate(ws.iter_rows(values_only=True)):
        ...
        # 获取基本数据
        glove_id = str(row[0])       # 手套ID
        level = int(row[1])          # 等级
        atk_mag = row[2] or 0        # 攻/魔
        def_mag_def = row[3] or 0    # 防/魔防
        ...
        # 添加到数据结构
        data[glove_id][f"等级{level}"] = level_data

    return data

def save_json(data, filename=r'C:\Users\李柱柱\Desktop\sc\scripts\材料新\json\手套奖励数据.json'):
    """保存为JSON文件"""
