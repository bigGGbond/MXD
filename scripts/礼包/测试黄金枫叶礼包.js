...
function action(mode, type, selection) {
    ...
    // 获取玩家购买进度
    var 购买进度 = cm.getOneTimeLog("黄金枫叶礼包购买进度") || 0;
    //cm.getPlayer().dropMessage(5,status);
    if (status == 0) {
        // 如果已经购买所有礼包
        if (购买进度 >= 礼包等级.length) {
            cm.sendOk("您已经购买了所有等级的黄金枫叶礼包，没有更多可购买的礼包了！\r\n\r\n");
            cm.dispose();
            return;
        }
        
        var text = "\t\t\t\t#e黄金枫叶礼包购买系统#n\r\n\r\n";
        text += "您可以在此处购买#r黄金枫叶礼包#k！\r\n\r\n";
       // text += "提示：每个等级的礼包只能购买一次，购买完当前等级才能解锁下一等级！\r\n\r\n";
        
        // 显示当前购买进度
        text += "您的购买进度: ";
        if (购买进度 == 0) {
            text += "#r尚未购买任何等级的黄金枫叶礼包#k\r\n";
        } else {
        ...
        var currentPackage = 黄金枫叶礼包数据.黄金枫叶[currentPrice];
        ...
        //     var nextPackage = 黄金枫叶礼包数据.黄金枫叶[nextPrice];
        ...
        var packageData = 黄金枫叶礼包数据.黄金枫叶[selectedPrice];
        
        // 确认购买
        cm.sendYesNo("您确定要购买 #r" + selectedPrice + "元宝#k 的黄金枫叶礼包吗？\r\n" +
                     "礼包内容:\r\n" +
        ...
        // 更新购买进度
        cm.deleteOneTimeLog("黄金枫叶礼包购买进度");
        cm.setOneTimeLog("黄金枫叶礼包购买进度", 购买进度 + 1);
        
        cm.sendOk("购买成功！您已获得 #r" + selectedPrice + "元宝#k 的黄金枫叶礼包。");
        status=-2;
        action(1,0,0);
        //cm.dispose();
    }
