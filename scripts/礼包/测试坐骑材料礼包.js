// 坐骑材料数据 - 每个数组第一个元素是材料ID，后面材料数量
var 一阶坐骑材料 = [4033062, 6, 10, 17, 36, 60]
var 二阶坐骑材料 = [4500902, 5, 9, 16, 33, 54]
var 三阶坐骑材料 = [4500903, 5, 8, 14, 30, 49]
var 四阶坐骑材料 = [4500904, 4, 7, 13, 27, 45]
var 五阶坐骑材料 = [4500905, 4, 7, 12, 24, 41]
var 六阶坐骑材料 = [4500906, 3, 6, 11, 22, 37]
var 七阶坐骑材料 = [4500907, 3, 6, 10, 20, 34]
var 八阶坐骑材料 = [4500908, 3, 5, 9, 18, 31]
var 九阶坐骑材料 = [4500909, 3, 5, 8, 17, 28]
var 十阶坐骑材料 = [4500910, 2, 4, 7, 15, 25]
var 十一阶坐骑材料 = [4500911, 2, 4, 7, 14, 23]
var 十二阶坐骑材料 = [4500912, 2, 3, 6, 13, 21]
var 十三阶坐骑材料 = [4500913, 2, 3, 6, 11, 19]

// 将所有坐骑材料数据组织成一个数组
var 坐骑材料列表 = [
    {name: "一阶坐骑材料", data: 一阶坐骑材料},
    {name: "二阶坐骑材料", data: 二阶坐骑材料},
    {name: "三阶坐骑材料", data: 三阶坐骑材料},
    {name: "四阶坐骑材料", data: 四阶坐骑材料},
    {name: "五阶坐骑材料", data: 五阶坐骑材料},
    {name: "六阶坐骑材料", data: 六阶坐骑材料},
    {name: "七阶坐骑材料", data: 七阶坐骑材料},
    {name: "八阶坐骑材料", data: 八阶坐骑材料},
    {name: "九阶坐骑材料", data: 九阶坐骑材料},
    {name: "十阶坐骑材料", data: 十阶坐骑材料},
    {name: "十一阶坐骑材料", data: 十一阶坐骑材料},
    {name: "十二阶坐骑材料", data: 十二阶坐骑材料},
    {name: "十三阶坐骑材料", data: 十三阶坐骑材料}
];
...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        var text = "\t\t\t\t#e坐骑材料礼包购买系统#n\r\n\r\n";
        text += "您可以在此处购买各种坐骑材料礼包！\r\n\r\n";
       //text += "提示：每个坐骑材料的每个等级礼包只能购买一次，购买完当前等级才能解锁下一等级！\r\n\r\n";
        
        // 用于生成礼包描述的函数
        var getPackageDesc = function(equipIndex, levelIndex) {
            var equipData = 坐骑材料列表[equipIndex].data;
        ...
        // 构建坐骑材料列表，每行显示两个equip
        for (var i = 0; i < 坐骑材料列表.length; i += 2) {
            var equip1 = 坐骑材料列表[i];
            var equip2 = (i + 1 < 坐骑材料列表.length) ? 坐骑材料列表[i + 1] : null;
            
            // 获取equip1的购买进度
            var progressKey1 = "坐骑材料礼包购买进度_" + i;
            ...
            //equip2行（如果存在）
            if (equip2) {
                var progressKey2 = "坐骑材料礼包购买进度_" + (i + 1);
        ...
        var equipData = 坐骑材料列表[selectedequip];
        ...
        // 获取该equip的购买进度
        var progressKey = "坐骑材料礼包购买进度_" + selectedequip;
