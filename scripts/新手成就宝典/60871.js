...
function start(mode, type, selection) {
    if (mode == -1) {
		...
		if (status == 0) {
			var text ="";
			text += "  升级一次每日戒指  \r\n";
            text += "  打开新手宝典-新手打造-传送到打造地图点击饰品打造NPC ,升级一次 #v1112422#   \r\n";
    ...
    if (mode == -1) {
        ...
        if (status == 0) {
			...
			if(qm.getInventory(1).getItem(1)== null ){
				qm.sendOk("装备栏第一格不是  #v1112422#");
				qm.dispose();
				return;
			}
			if(qm.getInventory(1).getItem(1).getItemId()!= 1112422 ){
				qm.sendOk("装备栏第一格不是  #v1112422#");
				qm.dispose();
				return;
			}
			if(qm.getInventory(1).getItem(1).getOwner() == "" || qm.getInventory(1).getItem(1).getOwner()==null){
				qm.sendOk("请去强化 #v1112422#");
				qm.dispose();
				return;
			}
