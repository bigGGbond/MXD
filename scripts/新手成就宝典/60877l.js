...
function start(mode, type, selection) {
    if (mode == -1) {
		...
		if (status == 0) {
			var text ="";
			text += "  升级武陵腰带  \r\n";
            text += "  快捷菜单-打造大厅-材料兑换-经验币兑换 ,升级一次 #v1132005#   \r\n";
...
function end(mode, type, selection) {
    if (mode == -1) {
        ...
        if (status == 0) {
			qm.循环播放动画("UI/tutorial.img/26",5000,3);
			...
			// if(qm.getInventory(1).getItem(1)== null ){
				// qm.sendOk("装备栏第一格不是  #v1132005#");
				// qm.dispose();
				// return;
			// }
			if(qm.getInventory(1).getItem(1).getItemId()!= 1132005 ){
				qm.sendOk("装备栏第一格不是  #v1132005#");
				qm.dispose();
				return;
			}
			if(qm.getInventory(1).getItem(1).getOwner() == "" || qm.getInventory(1).getItem(1).getOwner()==null){
				qm.sendOk("请去强化 #v1132005#");
				qm.dispose();
				return;
			}
			...
			// qm.gainItem(2022000,200);
			 // qm.gainItem(1122058,1);
			qm.forceCompleteQuest();
            qm.dispose();
			qm.openQuest(9900001,60878);
